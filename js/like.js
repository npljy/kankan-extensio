...
// cms视频收藏
var trStr =
  '<li class="{new}" style="{style}" title="{name}"><a href="{link}#kankan" target="_blank"><p><span>【片<span class="gap">片名</span>名】</span> {name}</p><p><span>【观看进度】</span> <span class="step">{step}</span><span class="time">【观看时间】</span> {time}</p></a></li>';
...
function initPage() {
  chrome.storage.local.get([KK_LIKE], function (result) {
    ...
    var domStr = jsList.map(function (e) {
      var str = trStr
        .replace("{link}", e.key || "#")
        .replaceAll("{name}", e.title || "无")
        .replace("{step}", e.step || "无")
        .replace("{update}", e.update || "无")
        .replace("{new}", e["new"] ? "new" : "")
        .replace("{time}", e.time || "无");
      if (e["new"])
        str = str.replace("{style}", "background: rgba(60, 197, 31, 0.1);");
      return str;
    });
    $(".main").html(domStr);
    bindDelete();
      ...
      $.ajax({
        ...
        success: (function () {
          var _success = _asyncToGenerator(
            /*#__PURE__*/ _regeneratorRuntime().mark(function _callee(resp) {
      ...
      chrome.storage.local.get(
        ...
        /*#__PURE__*/ (function () {
          var _ref = _asyncToGenerator(
            /*#__PURE__*/ _regeneratorRuntime().mark(function _callee2(result) {
